...
uint256 hashGenesisBlock("0x56ca023208cc39428692df7fecb4741051615f93fc08cd5c8b43071b265032c3");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // IRCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "IRCoin Signed Message:\n";
...
{
    // IRCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60;; // IRCoin: 1 days
static const int64 nTargetSpacing = 1 * 60; // IRCoin: 2 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // IRCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // IRCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // IRCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x56ca023208cc39428692df7fecb4741051615f93fc08cd5c8b43071b265032c3");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Feb 23, 2014: Sochi Olympics Closing Ceremony Latest Updates";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("047c8ad6f66a8fc8daab40cc2d69761329900a90895e18f0db595f23fa404fd81eb4ffc98d0c82bbae0504af8eaebc55360248dfbefbdbf9df407f7b7ade474d10") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1393397640;
        block.nBits    = 0x1e0fffff;
        block.nNonce   = 14921591;
        ...
        assert(block.hashMerkleRoot == uint256("0xf11f41702b386cea27ec0bc202bd2408b2c759ac8e76587f9cd856c5b62dcab9"));
...
unsigned char pchMessageStart[4] = { 0x06, 0x06, 0x07, 0x08 }; // IRCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("IRCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("IRCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static IRCoinMiner(CWallet *pwallet)
{
    printf("IRCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ircoin-miner");
    ...
    try { loop {
        ...
        printf("Running IRCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&IRCoinMiner, pwallet));
}
